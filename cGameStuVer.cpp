...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	gameTextList = { "Cat Fishing", "Score : "};
	strScore = gameTextList[1];
	strScore += to_string(theScore).c_str();
	
	theTextureMgr->addTexture("Title", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, gameTextList[0], textType::solid, { 0, 0, 0, 0 }, { 0, 0, 0, 0 }));
	theTextureMgr->addTexture("theScore", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, strScore.c_str(), textType::solid, { 0, 0, 0, 0 }, { 0, 0, 0, 0 }));
	...
	{
		//Changed Fish Position & direction 
		theAsteroids.push_back(new cAsteroid);
		theAsteroids[astro]->setSpritePos({ (200 * (astro + 1)+( 25* astro)), -75});
		theAsteroids[astro]->setSpriteTranslation({ 0, (-150 * (astro + 1) + (100 * astro)) });
...
void cGame::update(double deltaTime)
{
	...
	*/
	...
	if (frames % 75 == 0)
	{
		...
		{
			theAsteroids.push_back(new cAsteroid);
		    theAsteroids[astro]->setSpritePos({ (150 * count +( 25* (count -1))), -75});
		    theAsteroids[astro]->setSpriteTranslation({ 0, (-150 * count + (25 * (count - 1))) });
			int randAsteroid = AsteroidTextDis(gen);
			theAsteroids[astro]->setTexture(theTextureMgr->getTexture(textureName[randAsteroid]));
			theAsteroids[astro]->setSpriteDimensions(theTextureMgr->getTexture(textureName[randAsteroid])->getTWidth(), theTextureMgr->getTexture(textureName[randAsteroid])->getTHeight());
			theAsteroids[astro]->setAsteroidVelocity(200);
